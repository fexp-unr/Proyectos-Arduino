...
unsigned long tiempoBloqueo = 0; // defino la variable que va a registrar el tiempo en que sensor esté frente a un obstáculo
...
void loop () {
 if (digitalRead(inputPin) == HIGH){ //si el pin da una señal HIGH... (es decir, tiene un obstáculo enfrente)
  ...
  if(tiempoBloqueo == 0){
    tiempoBloqueo = millis() - tiempoInicial; //mido el tiempo que pasó entre el tiempo inicial y el momento actual (medio periodo)
    tiempoInicial = 0; // reseteo el tiempo inicial
  } else {
    tiempoBloqueo = millis() - tiempoInicial + tiempoBloqueo; //mido el tiempo que pasó entre el tiempo inicial y el momento actual (medio periodo)
    Serial.print("Tiempo de bloqueo: "); // doy salida de pantalla al tiempo medido
    Serial.print(tiempoBloqueo);
